        ...
        if self.video_capture.isOpened():
            ...
            if ret:
                ...
                if results.pose_landmarks:
                    ...
                    # Pega posição das mãos e cabeça
                    ...
                    # Desenha pontos no vídeo
                    self.mp_drawing.draw_landmarks(frame, results.pose_landmarks, self.mp_pose.POSE_CONNECTIONS)

                    # Checa se as duas mãos estão acima da cabeça (nariz)
                    if left_wrist_y < nose_y and right_wrist_y < nose_y:
                        if self.hands_up_start is None:
                            self.hands_up_start = time()
                        elif time() - self.hands_up_start > 5:
                            putText(frame, "ALERTA: Duas maos levantadas!", (50, 50),
                                    FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                            print("ALERTA: Duas mãos levantadas!")
                            self.ativar_sirene()  # <- você pode ativar a sirene aqui também
                            self.status_sirene = True

                    else:
                        self.hands_up_start = None

                if self.status_sirene == True:
